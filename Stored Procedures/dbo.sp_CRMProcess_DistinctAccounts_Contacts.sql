...
WHERE a.SeasonYear >=2016  --This was specific to Vikings, per request
GROUP BY b.SSB_CRMSYSTEM_ACCT_ID, b.SSB_CRMSYSTEM_CONTACT_ID, b.SourceSystem
-- DROP TABLE #tmpSTHs
-- SELECT * FROM #tmpSTHs

--Select DimcustomerID into #tmpPreCRM from Vikings.dbo.vwConversionContacts  --Old CRM
...
AND (SourceSystem LIKE 'CRM%'-- OR DimcustomerID in (select DimcustomerID from #tmpPreCRM) --Old CRM
)
